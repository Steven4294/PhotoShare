...
+(CGSize)messageSize:(NSString*)message {
    ...
    CGSize messageSize = [message sizeWithFont:[UIFont systemFontOfSize:messageTextSize] constrainedToSize:CGSizeMake([PTSMessagingCell maxTextWidth], CGFLOAT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
    if (messageSize.width < 22.5) {
        messageSize.width = 22.5;
...
+(UIImage*)balloonImage:(BOOL)sent isSelected:(BOOL)selected {
    ...
    //UIColor *color = [UIColor colorWithRed:240/255.0 green:240/255.0 blue:240/255.0 alpha:1.0f];
    ...
    normalBubble = [normalBubble resizableImageWithCapInsets:capInsets resizingMode:UIImageResizingModeStretch];
...
-(id)initMessagingCellWithReuseIdentifier:(NSString*)reuseIdentifier {
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {
       ...
       // self.messageLabel.font = [UIFont systemFontOfSize:messageTextSize];
        self.messageLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:messageTextSize];
        self.messageLabel.textColor = [UIColor colorWithRed:65/255.0 green:65/255.0 blue:65/255.0 alpha:1.0f];
...
- (void)layoutSubviews {
    ...
    if (self.sent == YES) {
        ...
        timeLabelFrame = CGRectMake(textMarginHorizontal, 0, dateSize.width, dateSize.height);
        
        ballonViewFrame = CGRectMake(textMarginHorizontal, 12, textSize.width, textSize.height);
        
        messageLabelFrame = CGRectMake(textMarginHorizontal, 12, textSize.width, textSize.height);
        
        avatarImageFrame = CGRectMake(self.frame.size.width - 55.0f, timeLabelFrame.size.height, 50.0f, 50.0f);

    } else {
        timeLabelFrame = CGRectMake(2*textMarginHorizontal + 35, 0.0f, dateSize.width, dateSize.height);
        
        ballonViewFrame = CGRectMake(5 + 40,  0, textSize.width + 12*2, textSize.height + 12*2);
        
        messageLabelFrame = CGRectMake(textMarginHorizontal + 45,  12, textSize.width, textSize.height);
        
        avatarImageFrame = CGRectMake(5.0f, timeLabelFrame.size.height, 40.0f, 40.0f);
  ...
  /*  self.messageLabel.layer.borderColor = [UIColor greenColor].CGColor;
    self.messageLabel.layer.borderWidth = 1.5f;
    
    self.balloonView.layer.borderColor = [UIColor orangeColor].CGColor;
    self.balloonView.layer.borderWidth = 1.5f;
    
    self.timeLabel.layer.borderColor = [UIColor lightGrayColor].CGColor;
    self.timeLabel.layer.borderWidth = 1.5f;*/
